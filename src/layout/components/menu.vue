<template>
  <div class="layout_menu flex h-[100vh] relative z-[1000]">
    <div class="w-[130px]" :class="style.theme == 'dark' ? 'bg-[#041527]' : 'bg-white'">
      ...
      >
        <SvgIcon class="w-[46px] h-[46px]" name="app/logo" />
      </div>
      <!-- 一级菜单以及二级菜单弹框  START -->
      <menuItem
    ...
    >
      <template v-if="style.showChildMenu">
        ...
        >
          <div
            :class="selectedKeys[2] == item.name ? 'text-primaryColor bg-[#EEF2FB] h-[40px] ' : ''"
...
const childMenu = computed(() => {
  if (selectedKeys.value.length == 3 && props.leftMenu.length > 0) {
    let child = props.leftMenu.find((e) => e.name == selectedKeys.value[1])!
    if (child && child.children) {
      return child.children.filter(
        (item: RouteRecordRaw) => auth.asyncRouter.includes(String(item.name)) && item.meta!.showMenu
      )
    } else {
      return []
    }
